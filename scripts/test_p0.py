...
#     print(f"\n{'='*60}")
#     print(f"{2 if bne_rounds==0 else 3}: {'Baseline' if bne_rounds==0 else f'P0 BNE {bne_rounds}round'} (10 episodes)")
#     print("="*60 + "\n")

#     import yaml, torch, os, re
...
#     with open("scripts/config_p0.yaml") as f:
...
#     memory_dim = int(getattr(args, 'memory_dim', getattr(args, 'belief_dim', 128) + 9))
...
#         if t:
#             traces.append(t)
#             if t.get('is_correct'): correct += 1
#         if (ep+1) % 5 == 0:
#             print(f"Episode {ep+1}/10: {'✓' if t.get('is_correct') else '✗'} 准确率={correct/(ep+1)*100:.1f}%")

#     with open(f"logs_p0_test_{tag}.json", "w") as f:
#         json.dump(traces, f)
...
from run_p0_test import run_testing
import argparse
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description="ECON test")
    parser.add_argument('--config', default=os.path.join(_SCRIPT_DIR, 'config_p0.yaml'))
    parser.add_argument('--test-eps', type=int, default=30, help='number of test episodes')
    parser.add_argument('--bne-rounds', type=int, default=0, help='number of BNE rounds (0 for no BNE)')
    parser.add_argument('--log-dir', default='logs_0_gsm8k')
    parser.add_argument('--model-dir', default='models_0_gsm8k')
    ...
    run_testing(args.config, args.test_eps, args.bne_rounds, args.log_dir, args.model_dir, args.tag, logger)
