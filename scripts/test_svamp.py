# #!/usr/bin/env python3
# """
# Test a trained ECON model on the SVAMP dataset (baseline vs BNE).
# """
# import os
# import sys
# import json
...
# MODEL_DIR = "./models_svamp/final"


# def test(bne_rounds, tag):
#     print(f"\n{'='*60}")
#     mode = "Baseline" if bne_rounds == 0 else f"SVAMP BNE {bne_rounds}round"
...
#     with open("scripts/config_svamp.yaml") as f:
...
#     with open(f"logs_svamp_test_{tag}.json", "w") as f:
...
from test_p0 import main as base_main  # noqa: E402
...
def main():
    ...
    if "--log-dir" not in sys.argv:
        sys.argv.extend(["--log-dir", os.path.join(PROJECT_ROOT, "logs_0_svamp")])
    if "--model-dir" not in sys.argv:
        sys.argv.extend(["--model-dir", os.path.join(PROJECT_ROOT, "models_0_svamp")])
    if "--bne-rounds" not in sys.argv:
        sys.argv.extend(["--bne-rounds", "0"])
    base_main()
